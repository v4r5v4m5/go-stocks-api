...
func CreateStock(w http.ResponseWriter, r *http.Request) {
	...
	// golang doesn't understand json we need to decode it
	err := json.NewDecoder(r.Body).Decode(&stock) // decode body and decode into stock

	// handle errors while decoding
	if err != nil {
		log.Fatalf("[-] Unable to decode the request body %v", err)
	}

	insertId := insertStock(stock) // insert stock basically communicates to the database for insertion. Database level function.
	...
	// when we send the response we need to encode it into json
...
func getStock(id int) (models.Stock, error) {
	...
	case nil:
		return stock, nil
	default:
		log.Fatalf("[-] Unable to scan the row no. %v", err)
	}
